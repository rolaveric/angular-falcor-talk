  ...
  falcor.Model.prototype.getViewValue = function(path) {
    if (path) {
      ...
      if (angular.isArray(path)) {
        pathString = path.reduce((memo, token, index) => {
          token = String(token);
          if (!isNaN(token)) {
            token = '[' + token + ']';
